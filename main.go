...
func sendDiscordAlert(webhookURL, mapsAPIKey string, incident UnifiedIncident) (string, error) {
	if incident.Source == "NCDOT" {
		return sendNcdotAlert(webhookURL, mapsAPIKey, incident)
	} else if incident.Source == "RWECC" {
		return sendRweccAlert(webhookURL, mapsAPIKey, incident)
	}
	return "", fmt.Errorf("unknown incident source: %s", incident.Source)
}

// sendNcdotAlert formats and sends a specialized embed for NC DOT crashes.
func sendNcdotAlert(webhookURL, mapsAPIKey string, incident UnifiedIncident) (string, error) {
	...
	if err := json.Unmarshal(incident.Details, &ncdotDetails); err != nil {
		return "", fmt.Errorf("error unmarshalling NCDOT details for alert: %w", err)
	}
...
// sendRweccAlert formats and sends a specialized embed for RWECC incidents.
func sendRweccAlert(webhookURL, mapsAPIKey string, incident UnifiedIncident) (string, error) {
	var rweccDetails struct {
		Problem      string `json:"problem"`
		Jurisdiction string `json:"jurisdiction"`
	}
	if err := json.Unmarshal(incident.Details, &rweccDetails); err != nil {
		return "", fmt.Errorf("error unmarshalling RWECC details for alert: %w", err)
	}
...
func updateDiscordAlert(webhookURL, messageID string, incident UnifiedIncident) error {
	...
	if err != nil {
		return fmt.Errorf("error sending PATCH request to Discord: %w", err)
	}
	defer resp.Body.Close()
...
func main() {
	...
	if webhookURL == "" {
		log.Fatalln("Error: DISCORD_HOOK must be set in your environment or .env file.")
	}
	...
	for rows.Next() {
		...
		if err := rows.Scan(&i.ID, &i.Source, &i.SourceID, &i.EventType, &i.Address, &i.Latitude, &i.Longitude, &i.Timestamp, &i.Details); err != nil {
			log.Printf("Error scanning incident from DB: %v", err)
			continue
		}

		if !sentIDs[i.ID] {
			log.Printf("Found new unified incident from %s (ID: %s). Sending alert.", i.Source, i.SourceID)
			messageID, err := sendDiscordAlert(webhookURL, mapsAPIKey, i)
			if err != nil {
			...
			if err != nil {
				log.Printf("Error saving discord_message_id to DB: %v", err)
			}
	...
	for clearedRows.Next() {
		...
		if err := clearedRows.Scan(&i.ID, &i.Source, &i.Address, &i.DiscordMessageID); err != nil {
			log.Printf("Error scanning cleared incident from DB: %v", err)
			continue
		}

		log.Printf("Found cleared incident from %s (ID: %d). Updating Discord message.", i.Source, i.ID)
		...
		if err != nil {
			log.Printf("Error nullifying discord_message_id in DB: %v", err)
		}
